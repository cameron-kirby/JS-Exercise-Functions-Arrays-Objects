 ...
 * }
*/
function makePersonObject(iden, nameIn, emailIn) {
  let newPerson = {
    id: iden,
    name: nameIn,
    email: emailIn
...
*/
function appleIndex(stringList) {
  let index;
  for(let i = 0; i < stringList.length; i++){
    if(stringList[i] === "apple") {
      index = i;
    }
  }
  return index;
...
*/
function isItAnApple(stringList) {
  let truthList = [];
  for(let i = 0; i < stringList.length; i++){
    if(stringList[i] === "apple") {
      truthList.push(true);
...
let newObject = {
  ...
  cat: "tabby",
  frog: "tree",
  getDog: function(num1, num2){
    return num1 + num2;
  }
}

console.log(newObject.egg(2,4));
...
function getLastCarInfo(inventory) {
  ...
  const vehicle = inventory[lastCarIndex];
  return `This is a ${vehicle.car_make} ${vehicle.car_model}` ;
}
